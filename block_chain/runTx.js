...
module.exports = function (opts, cb) {
  ...
  function runCall (cb) {
    ...
    // 操作fromAccount对应的account的balance（预扣款）
    ...
    function parseResults (err, _results) {
      ...
      // generate the bloom for the tx logs，布隆过滤器用于快速判断某一个元素是否存在（占用空间非常小）
      ...
      function updateFromAccount (next) {
        ...
        self.stateManager.putAccountBalance(util.toBuffer(tx.from), finalFromBalance, next)
      }
      ...
      function cleanupAccounts (next) {
        ...
        // get being destoried contract address

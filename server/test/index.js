...
describe("process transaction", function() {
  ...
  it("step 8", function(done) {
    ...
    processor.blockChain.stateManager.getAccount(address, function(err, account) {
      assert(!err, "err");
      assert(account.balance.toString("hex") === "1976");
      done();
    });
  });

  it("step 9", function(done) {
    this.timeout(10000);

    processor.blockChain.stateManager.getAccount(util.toBuffer("0x1234567891123456789112345678911234567891"), function(err, account) {
      assert(!err, "err");
      assert(account.balance.toString("hex") === "a68a");
      done();
    });
  });
});

  ...
  <div class='container'>
    <div style="width:100%;">
      <el-card shadow="always">
        <h2>{{`${currentNode.host}:${currentNode.port}`}}</h2>
      </el-card>
    </div>
    
    <div class="border" style="width:100%;height:500px;overflow:auto;margin:20px 0px 20px 0px;padding-top:20px;">
      <div style="justify-content:left;" v-bind:key="nodeInfo.id" v-for="nodeInfo in nodesInfo">
        <div style="display: flex;flex-direction:row;justify-content:start;cursor:pointer;" @dblclick="chooseNode(nodeInfo)">
          ...
          <p style="width:100%;">{{`${nodeInfo.host}:${nodeInfo.port}`}}</p>
        </div>
        <div style="display: flex;flex-direction:row;justify-content:start;">
          <span style="width:100px;">区块哈希值</span>
          <p style="width:100%;">{{nodeInfo.detail.hash}}</p>
        </div>
        <div style="display: flex;flex-direction:row;justify-content:start;">
          <span style="width:100px;">区块链高度</span>
          <p style="width:100%;">{{nodeInfo.detail.number}}</p>
        </div>
